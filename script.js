...
(function () {
    ...
    function checkScratchProgress(ctx, canvas, index, cardEl) {
        ...
        if (pct >= CONFIG.scratchThreshold && !cardEl._revealed) {
            ...
            // Check if all cards revealed — unlock scroll
            if (revealedCount >= TOTAL_CARDS) {
                unlockScrollGate();
            }
        }
    }
    ...
    /* ═══════════════════════════════════════════════════
       9. SCROLL GATE — LOCK UNTIL SCRATCH CARDS DONE
       ═══════════════════════════════════════════════════ */
    function initScrollGate() {
        const scratchSection = $('#scratch-section');
        const gateHint = $('#scroll-gate-hint');
        if (!scratchSection || !gateHint) return;

        // Use ScrollTrigger to detect when user reaches scratch section
        ScrollTrigger.create({
            trigger: scratchSection,
            start: 'top 60%',
            onEnter: () => {
                if (revealedCount < TOTAL_CARDS && !scrollGateActive) {
                    scrollGateActive = true;
                    // Lock scroll on body
                    document.body.classList.add('scroll-locked');
                    // Scroll to the scratch section top
                    scratchSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
                    // Show hint
                    gateHint.classList.add('visible');
                }
            }
        });

        // Also block scroll wheel / touchmove when locked
        document.addEventListener('wheel', preventScrollWhenLocked, { passive: false });
        document.addEventListener('touchmove', preventScrollWhenLocked, { passive: false });
    }

    function preventScrollWhenLocked(e) {
        if (!scrollGateActive) return;
        // Allow touch events on scratch canvases
        const target = e.target;
        if (target.classList && target.classList.contains('scratch-canvas')) return;
        e.preventDefault();
    }

    function unlockScrollGate() {
        scrollGateActive = false;
        document.body.classList.remove('scroll-locked');
        const gateHint = $('#scroll-gate-hint');
        if (gateHint) {
            gateHint.classList.remove('visible');
            gateHint.classList.add('completed');
        }
    ...
    function startAudio() {
        ...
        const iconOn = toggleBtn ? toggleBtn.querySelector('.audio-icon-on') : null;
        const iconOff = toggleBtn ? toggleBtn.querySelector('.audio-icon-off') : null;
        ...
        if (playPromise !== undefined) {
            playPromise.then(() => {
                // Show toggle button with playing state
                if (toggleBtn) {
                ...
                // Show button in muted state for manual play
                if (toggleBtn) {
    ...
    function initAudio() {
        ...
        // ── Visibility Change: Pause/Resume on tab switch or app minimize ──
        document.addEventListener('visibilitychange', () => {
            ...
            if (document.hidden) {
                ...
                // Page is visible again — resume only if user hasn't manually muted
                if (!userMuted) {
                    audio.play().then(() => {
                        iconOn.style.display = '';
                        iconOff.style.display = 'none';
